...
class MCSimulation:
    ...
    def calc_cumulative_return(self):
        ...
        for n in range(self.nSim):
            ...
            portfolio_cumulative_returns = pd.concat([portfolio_cumulative_returns, sim_df], axis=1)

        portfolio_cumulative_returns.columns = range(0, self.nSim)
        portfolio_cumulative_returns = (1 + portfolio_cumulative_returns.fillna(0)).cumprod()
    ...
    def plot_simulation(self, **kwargs):
        """
        ...
        cond       = ("legend" in kwargs.keys(), "title" in kwargs.keys())

        plot_params_switch = {
            #legend, title
            (False,  False): lambda : self.simulated_return.plot(legend=None,title=plot_title, **kwargs),
            (False,  True):  lambda : self.simulated_return.plot(legend=None, **kwargs),
            (True,   False): lambda : self.simulated_return.plot(title=plot_title, **kwargs),
            (True,   True):  lambda : self.simulated_return.plot(**kwargs),
		}

        return plot_params_switch.get(cond)()
    ...
    def plot_distribution(self, **kwargs):
        ...
        cond       = "title" in kwargs.keys()

        # Remove invalid keyword arguments and keep default
        for key, val in {"kind": "hist", "bins": 10, "density": True}.items():
            if key in kwargs.keys():
                warn_str = f'Key "{key}" must be "{val}"! Cannot alter to "{kwargs[key]}".'
                warnings.warn(warn_str, UserWarning, stacklevel=1)
                kwargs.pop(key, None)

        # Create histogram plot
        plot_params_switch = {
            #title
            True:  lambda : self.simulated_return.iloc[-1, :].plot(kind='hist', bins=10, density=True, **kwargs),
            False: lambda : self.simulated_return.iloc[-1, :].plot(kind='hist', bins=10, density=True, title=plot_title, **kwargs),
		}

        plt = plot_params_switch.get(cond)()
